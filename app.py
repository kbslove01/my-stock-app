...
st.write("데이터 수집 방식 최적화 완료! (장마감 정산 시간대 우회 적용)")

# -----------------------------------------------------------------------------
# [데이터 수집 및 필터링 함수 (강력 방어형)]
# -----------------------------------------------------------------------------
...
def get_and_filter_stocks(max_price, max_pbr, max_per, min_div):
    ...
    # 1. 오후 3시 ~ 6시 사이는 KRX 데이터 업데이트로 불안정하므로 안전한 전날 데이터부터 탐색
    if 15 <= now.hour < 18:
        check_date = now - dt.timedelta(days=1)
    else:
        check_date = now
    ...
    # 2. 최근 7일 중 코스피/코스닥 데이터가 완벽하게 존재하는 날짜 찾기
    for _ in range(7):
        target_str = check_date.strftime("%Y%m%d")
        try:
            # 코스피, 코스닥 각각 가져와서 물리적으로 합치기 (ALL 옵션 에러 원천 차단)
            p_kospi = stock.get_market_ohlcv(target_str, market="KOSPI")
            p_kosdaq = stock.get_market_ohlcv(target_str, market="KOSDAQ")
            p_df = pd.concat([p_kospi, p_kosdaq])
            
            f_kospi = stock.get_market_fundamental(target_str, market="KOSPI")
            f_kosdaq = stock.get_market_fundamental(target_str, market="KOSDAQ")
            f_df = pd.concat([f_kospi, f_kosdaq])
            
            # 주가와 재무 데이터가 모두 정상적으로 존재하는 날짜를 찾으면 스톱!
            if not p_df.empty and not f_df.empty and '종가' in p_df.columns and 'PBR' in f_df.columns:
                df_price = p_df
                df_fundamental = f_df
                break
        except:
    ...
    if df_price.empty or df_fundamental.empty:
        st.error("한국거래소(KRX)에서 데이터를 가져오지 못했습니다. 잠시 후 다시 시도해 주세요.")
        return pd.DataFrame()
    ...
    try:
        ...
        df = df[df['PBR'] > 0] # PBR 누락/오류 종목 제외
...
def draw_analysis_chart(ticker_code, ticker_name):
    ...
    fig.update_layout(title=f"📈 {ticker_name} ({ticker_code}) 기술적 분석 차트", yaxis_title='주가 (원)', xaxis_rangeslider_visible=False, xaxis=dict(range=[zoom_start, df_chart.index[-1]]), height=600, template="plotly_white")
    return fig
...
if btn_search:
    ...
    with st.spinner("KRX 데이터를 꼼꼼하게 분석 중입니다..."):
    ...
    with col_table:
        ...
        if not df_result.empty:
            ...
            if selected_rows:
                ...
                with col_chart: st.info("👈 표에서 종목을 클릭하시면 이동평균선 차트가 나타납니다.")
        else: 
            st.warning("조건을 만족하는 주식이 없습니다. (검색 조건은 완벽합니다!)")
else:
    with col_table: st.info("👈 조건을 입력한 후 [조건으로 주식 검색] 버튼을 눌러주세요.")
